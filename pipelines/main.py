...
@dlt.table(
  comment="Raw trips data without any additional transformations"
)
def trips_raw():
...
@dlt.table(
  comment="Only trips which are considered as a group (3 or more passangers)"
)
def group_trips():
...
@dlt.view(
  comment="Group trips extended with pickup information"
)
...
def group_trips_with_pickup_and_holiday_info():
    return GroupsReportProvider.add_holiday_info(dlt.read("group_trips_with_pickup_info"))
